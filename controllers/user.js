...
function resetUserforAdmin(req, res) {
    var params = req.body;
    var userId = req.params.id;
    var pass = bcrypt.hashSync( params.password, 10 );
    ...
    User.findByIdAndUpdate( userId, newPassword, { new: true }, (err, userReset) => {
        if(err){
            return res.status(500).send({
                ok: false,
                err
